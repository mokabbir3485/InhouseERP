...
namespace Security.UI.Controllers
{
    public class CompanyController : Controller
    {
        ...
        [HttpGet]
        public JsonResult GetAllOrgnazition()
        {
            try
            {
                var localGetById = Facade.CompanyBLL.GetAllOrgnazition();
        ...
        [HttpGet]
        public JsonResult GetCompanyAddresses(int companyId)
        {

            try
            {
                var list = Facade.CompanyAddressBLL.GetCompanyAddresses(companyId);
                return Json(list, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
       ...
       [HttpGet]
       public JsonResult GetCompanyBillDeliveryAddressByCompanyId(Int32 CompanyId)
        {

            try
            {
                var list = Facade.CompanyBLL.GetCompanyBillDeliveryAddressByCompanyId(CompanyId);
                return Json(list, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public int SaveCompany(ad_Company _ad_Company, List<ad_CompanyAddress> _ad_CompanyAddressList, List<ad_CompanyBillPolicy> ad_CompanyBillPolicyList)
        {
            ...
            try
            {
                ...
                {                    
                    ...
                    companyId = ret;
                    if(_ad_Company.IsSupplier == true && _ad_Company.SupplierId > 0)
                    {
                        _ad_Company.CompanyId = ret;
                        Facade.CompanyBLL.CompanyWiseSupplierPost(_ad_Company);
                    }
                ...
                foreach (var companyAddress in _ad_CompanyAddressList)
                {
                    ...
                    if (String.IsNullOrEmpty(companyAddress.TIN))
                        companyAddress.TIN = "";
                    if (String.IsNullOrEmpty(companyAddress.BIN))
                        companyAddress.BIN = "";
