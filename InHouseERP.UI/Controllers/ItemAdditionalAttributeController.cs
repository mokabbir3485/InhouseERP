...
namespace Security.UI.Controllers
{
    public class ItemAdditionalAttributeController : Controller
    {
        ...
        [HttpPost]
        public int SaveItemAdditionalAttribute(ad_ItemAdditionalAttribute ItemAdditionalAttribute)
        {
            
            int ret = 0;
            try
            {
                //if (ItemAdditionalAttribute.IsActive == true )
                //{
                    if (ItemAdditionalAttribute.ItemAddAttId == 0)
                    {
                    ...
                    {
                        ret = Facade.ItemAdditionalAttribute.Update(ItemAdditionalAttribute);
                    }
                //}
                //else
                //{
        ...
        public JsonResult GetItemAdditionalAttributeByItemId(int ItemId)
        {
            try
            {
                var list = Facade.ItemAdditionalAttribute.GetItemAdditionalAttributeByItemId(ItemId);
                return Json(list, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetAllControlHead()
        {
            try
            {
                var list = Facade.ItemAdditionalAttribute.GetAllControlHead();
                return Json(list, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
