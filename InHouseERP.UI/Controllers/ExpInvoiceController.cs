...
namespace Security.UI.Controllers
{
    public class ExpInvoiceController : Controller
    {
        ...
        public JsonResult GetInvoicePaged(int startRecordNo, int rowPerPage, string whereClause, int rows)
        {
            ...
            try
            {
                ...
                var customMODEntity = new
                {
                    ...
                    ListData = Facade.exp_Invoice.GetPaged(startRecordNo, rowPerPage, whereClause, "InvoiceId", "DESC", ref rows),
                    TotalRecord = rows
                };
        ...
        public long GetInvoiceNo(Int32 ExporterId)
        {
            try
            {
                long maxNumber;
                maxNumber = Facade.exp_Invoice.GetMaxInvoiceNo(ExporterId);
                return maxNumber;
            }
            catch (Exception ex)
            {
        ...
        public string Post(exp_Invoice exp_Invoice, List<exp_InvoiceDetail> invoiceDetailList, exp_PackingInfo exp_PackingInfo, exp_InvoiceDetail_TableHtml tableHtmlData, List<exp_InvoiceDetail_ModifiedData> modifiedDataList, List<exp_POReference> POReferencelist)
        {
            ...
            try
            {
                using (TransactionScope ts = new TransactionScope())
                {
                    ...
                    invoiceId = Convert.ToInt64(ret);
                    tableHtmlData.InvoiceId = Convert.ToInt64(ret);
					...
					{
						if (item.ItemDescriptionTwo == null)
							item.ItemDescriptionTwo = "";
                        if (item.DescriptionTwo == null)
							item.DescriptionTwo = "";
                        if (item.ItemName == null)
							item.ItemName = "";
						if (item.CartonSize == null)
							item.CartonSize = "";
						item.InvoiceId = Convert.ToInt64(ret);
                        Facade.exp_Invoice.AddDetail(item);
					}

					if (modifiedDataList.Count != 0)
					{
						Facade.exp_Invoice.DeleteInvoiceDetailModifiedData(Convert.ToInt64(ret));
					}
					foreach (var modifiedData in modifiedDataList)
					{
						...
						modifiedData.InvoiceId = Convert.ToInt64(ret);
                    ...
                    if (POReferencelist != null && POReferencelist.Count > 0 && invoiceId > 0)
                    {
                        foreach (exp_POReference aPOReferencelist in POReferencelist)
                        {
                            ...
                            aPOReferencelist.DocumentId = Convert.ToInt64(ret);
					...
					exp_PackingInfo.InvoiceId = Convert.ToInt64(ret);
