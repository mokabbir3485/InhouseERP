...
namespace Security.UI.Controllers
{
    public class BillOfMaterialController : Controller
    {
        ...
        public JsonResult GetMaxBillOfMaterialNo()
        {
            try
            {
                var maxNumber = Facade.inv_BillOfMaterialBLL.GetMaxBillOfMaterialNo();
        ...
        public JsonResult GetBillOfMaterialPaged(int startRecordNo, int rowPerPage, string whereClause, int rows)
        {
            ...
            try
            {
                ...
                var customMODEntity = new
                {
                    ...
                    ListData = Facade.inv_BillOfMaterialBLL.GetPaged(startRecordNo, rowPerPage, whereClause, "BillOfMaterialId", "DESC", ref rows),
                    TotalRecord = rows
                };
        ...
        public string Save(inv_BillOfMaterial BillOfMaterial, inv_BillOfMaterialDetail[] BillOfMaterialDetaillist, inv_BillOfMaterialOverhead[] OverheadDetailList)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (BillOfMaterialDetaillist != null)
                    {
                        Facade.inv_BillOfMaterialBLL.DeleteDetailByBOMId(Convert.ToInt64(ret));
                        foreach (var BOMDetail in BillOfMaterialDetaillist)
                        {
                    ...
                    if (OverheadDetailList != null)
                    {
                        Facade.inv_BillOfMaterialBLL.DeleteOerheadByBOMId(Convert.ToInt64(ret));
                        foreach (var OverheadDetail in OverheadDetailList)
                        {
