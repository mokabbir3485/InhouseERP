...
namespace Security.UI.Controllers
{
    public class ItemController : Controller
    {
        ...
        [HttpGet]
        public JsonResult GetByMatrialIdWithCurrentStock(Int32 MatrialTypeId, Int32 departmentid)
        {
            try
            {
                var list = Facade.Item.GetByMatrialIdWithCurrentStock(MatrialTypeId, departmentid);
                return Json(list, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        //}
        [HttpPost]
        public int Post(ad_Item item,ad_ItemVat itemVat, List<ad_Item> TagRibbonWithLabel, List<ad_Item> DeleteTagRibbonWithLabel)
        {
            ...
            try
            {
                using (System.Transactions.TransactionScope ts = new System.Transactions.TransactionScope())
                {
                    ...
                    ret = Facade.Item.Post(item);

                    if (TagRibbonWithLabel != null && TagRibbonWithLabel.Count > 0)
                    {
                        foreach (ad_Item aTagRibbonWithLabel in TagRibbonWithLabel)
                        {
                            Int64 Sp_ret;

                            aTagRibbonWithLabel.RibbonId = ret;
                            aTagRibbonWithLabel.LabelId = aTagRibbonWithLabel.ItemId;
                            Sp_ret = Facade.Item.PostSpecialItem(aTagRibbonWithLabel);
                        }
                    }

                    if (DeleteTagRibbonWithLabel != null && DeleteTagRibbonWithLabel.Count > 0)
                    {
                        foreach (ad_Item aDeleteTagRibbonWithLabel in DeleteTagRibbonWithLabel)
                        {

                            Facade.Item.DeleteRibbonInPerLabelById(aDeleteTagRibbonWithLabel.Id);
                        }
                    }
                    //TagRibbonWithLabel
                    //TagRibbonWithLabel
                    ...
                    {
                        itemVat.ItemId = ret;
                        Facade.Item.ad_ItemVat_Add(itemVat);
                    }
                    else
                    {
        ...
        [HttpGet]
        public JsonResult GetItemStatusDetailByItemId(Int64 ItemId)
        {
          
            try
            {

              var  itemVatList = Facade.Item.GetItemStatusDetailByItemId(ItemId);
                return Json(itemVatList, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult GetRibbonInPerLabelCarton_GetAllLabel(int? RibbonId = null)
        {
          
            try
            {

              var  itemList = Facade.Item.GetRibbonInPerLabelCarton_GetAllLabel(RibbonId);
        ...
        [HttpGet]
        public JsonResult GetByTopItem(int itemId)
        {
            try
            {
                var localGetById = Facade.Item.GetByTopItem(itemId);
        ...
        public JsonResult GetCompanyWiseItemForSO(int ItemId, int CompanyId)
        {
            try
            {
                var list = Facade.Item.GetCompanyWiseItemForSO(ItemId, CompanyId);
                return Json(list, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetItemForSo()
        {
            try
            {
                var list = Facade.Item.GetItemForSo();
                return Json(list, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetItemSearchResultPaged(int StartRecordNo, int RowPerPage, string whClause, int rows)
        {
            try
            {
                ...
                {
                    ListData = Facade.Item.GetPaged(StartRecordNo, RowPerPage, whClause, "I.UpdateDate", "DESC", ref rows),
                    TotalRecord = rows
                };
        ...
        //    }
        //}
        public JsonResult SaveFiles([System.Web.Http.FromUri] string SODId, [System.Web.Http.FromUri] string ArtFileName)
        {
            ...
            if (Request.Files != null)
            {
                ...
                // var artWorkFileName ="Art" + "_" + SODId + "_" + actualFileName;


                try { 
                
                    file.SaveAs(Path.Combine(Server.MapPath("~/UploadedFiles/ArtWork"), SODId));
        ...
        public JsonResult ArtDeleteFile(string FileName)
        {
            try
            {
                //DateTime date = DateTime.Now;
        ...
        [HttpGet]
        public JsonResult GetPagedItemStatus(int startRecordNo, int rowPerPage, string whereClause, int rows)
        {
            try
            {
                ...
                var customMODEntity = new
                {
                    ListData = Facade.Item.GetPagedItemStatus(startRecordNo, rowPerPage, whereClause, "CID.ItemId", "DESC", ref rows),
                    TotalRecord = rows
                };
                return Json(customMODEntity, JsonRequestBehavior.AllowGet);
            }
