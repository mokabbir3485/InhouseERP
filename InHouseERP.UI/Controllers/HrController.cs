...
{
    [EnableCors(origins: "http://localhost:4200,http://202.126.122.115:85,http://192.168.0.23:85", headers: "*", methods: "*")]
    public class HrController : ApiController
    {
        ...
        [Route("Employee/GetLeaveBalance")]
        [HttpGet]
        public IEnumerable<hr_LeaveBalance> GetLeaveBalance(int YearId, int BranchId, int GradeId, int DepartmentId, int SectionId, int EmployeeId)
        {
            List<hr_LeaveBalance> hr_LeaveBalanceList = new List<hr_LeaveBalance>();
            try
            {
                hr_LeaveBalanceList = HrAndPayrollBLL.Facade.hr_LeaveTypeSetup.GetLeaveBalance(YearId, BranchId, GradeId, DepartmentId, SectionId, EmployeeId);
            }
            catch (Exception ex)
            {

            }
            return hr_LeaveBalanceList;
        }

        [Route("SalaryDetail/GetAllSalaryDetail")]
        [HttpGet]
        public IEnumerable<hr_SalaryDetailReport> GetAllSalaryDetail(int MonthId, int YearId, int GradeId, string BranchName, string UnitName, string DepartmentName, string Header1, string Header2, string Header3)
        {
            List<hr_SalaryDetailReport> hr_SalaryDetailList = new List<hr_SalaryDetailReport>();
            try
            {
                hr_SalaryDetailList = HrAndPayrollBLL.Facade.hr_SalaryDetail.GetAllSalaryDetail(MonthId, YearId, GradeId, BranchName, UnitName, DepartmentName, Header1, Header2, Header3);
            }
            catch (Exception ex)
            {

            }
            return hr_SalaryDetailList;
        }
        [Route("Employee/GetAllSalaryHistory")]
        [HttpGet]
        public IEnumerable<hr_SalaryHistory> GetAllSalaryHistory(int FromMonthYearId, int ToMonthYearId, int BranchId, int GradeId, int DepartmentId, int SectionId, int EmployeeId)
        {
            List<hr_SalaryHistory> hr_SalaryDetailList = new List<hr_SalaryHistory>();
            try
            {
                hr_SalaryDetailList = HrAndPayrollBLL.Facade.hr_SalaryDetail.GetAllSalaryhistory(FromMonthYearId, ToMonthYearId, BranchId, GradeId, DepartmentId, SectionId, EmployeeId);
            }
            catch (Exception ex)
            {
        ...
        {
            List<ad_Department> departmentList = SecurityBLL.Facade.Department.GetAllActiveByBranchId(branchId);
        ...
        {
            List<ad_Department> departmentList = SecurityBLL.Facade.Department.GetByBranchAndGrade(branchId,gradeId);
            return departmentList;
        }
        
        [Route("WagesSlip/WagesSlipForHr/{MonthId}/{YearId}/{GradeId}/{BranchName}/{DepartmentIds}/{SectionIds}")]
        [HttpGet]
        public IEnumerable<hr_SalaryDetail> WagesSlipForHr(int MonthId, int YearId, int GradeId, string BranchName, string DepartmentIds, string SectionIds)
        {
            List<hr_SalaryDetail> WagesSlipList = HrAndPayrollBLL.Facade.hr_SalaryDetail.WagesSlipForHr(MonthId, YearId, GradeId, BranchName, DepartmentIds, SectionIds);
            return WagesSlipList;
        }
        [Route("PaySlip/PaySlipForHr/{MonthId}/{YearId}/{GradeId}/{BranchName}/{DepartmentIds}/{SectionIds}")]
        [HttpGet]
        public IEnumerable<hr_SalaryDetail> PaySlipForHr(int MonthId, int YearId, int GradeId, string BranchName, string DepartmentIds, string SectionIds)
        {
            List<hr_SalaryDetail> PaySlipList = HrAndPayrollBLL.Facade.hr_SalaryDetail.PaySlipForHr(MonthId, YearId, GradeId, BranchName, DepartmentIds, SectionIds);
            return PaySlipList;
        }
        ...
        {
            List<ad_DepartmentWiseSection> sectionList = SecurityBLL.Facade.ad_DepartmentWiseSection.GetSectionByDepartmentId(departmentId);
            return sectionList;
        }
       ...
       // }
        ...
        public IEnumerable<ad_Branch> GetAllBranch(string where = "1=1 and isActive=1", string orderBy = "BranchId")
        {
            ...
            List<ad_Branch> branchList = SecurityBLL.Facade.Branch.ad_BranchDAO.GetDynamic(where, orderBy);
            return branchList;
        }


        [HttpGet]
        [Route("HrTypes/GetAllScreen")]
        public List<s_Screen> GetAllScreen()
        {
            List<s_Screen> ScreenList = SecurityBLL.Facade.Screen.GetAll();
            for (int i = 0; i < ScreenList.Count; i++)
            {
                List<s_ScreenDetail> ScreenDetailList = SecurityBLL.Facade.ScreenDetail.GetByScreenId(ScreenList[i].ScreenId);
                ScreenList[i].ScreenDetailList = ScreenDetailList;
            }
        ...
        [Route("HRPermision/PermisionGetPaged")]
        [HttpGet]
        public IHttpActionResult PermisionGetPaged(int startRecordNo, int rowPerPage, string whereClause, int rows)
        {
            string whereClause1 = "";
             whereClause1 = whereClause;
           // var ListData = InventoryBLL.Facade.inv_InternalWorkOrderBLL.GetPaged(startRecordNo, rowPerPage, whereClause, "InternalWorkOrderId", "DESC", ref rows);
            var customMODEntity = new
            {
                ListData =SecurityBLL.Facade.Permission.GetPaged(startRecordNo, rowPerPage, whereClause1, "ScreenId", "DESC", ref rows),
                TotalRecord = rows
            };
            ...
            return Ok(customMODEntity);
        }


        [Route("HRUser/UserGetPaged")]
        [HttpGet]
        public IHttpActionResult UserGetPaged(int startRecordNo, int rowPerPage, string whereClause, int rows)
        {
            string whereClause1 = "";
            whereClause1 = whereClause;
            var customMODEntity = new
            {
                ListData = SecurityBLL.Facade.User.HrUserGetPaged(startRecordNo, rowPerPage, whereClause1, "HrUserId", "ASC", ref rows),
                TotalRecord = rows
            };
