app.controller("RoleEntryController", function ($scope, $rootScope, $http, $window) {
    Clear();
    ...
    function Clear() {
        ...
        $scope.ScreenId = Permission.find(v => v.ScreenName == 'Role').ScreenId;
    ...
    $scope.reloadBtn = function () {
        $('#textUserGroupName').val('');
        $scope.UserGroupName = null;
        GetAllRolePaged(1);
    }
    $scope.UserGroupNameSearch = function () {
        GetAllRolePaged(1);
    ...
    function GetAllRolePaged(curPage) {
        ...
        var whereClause = '[IsSuperAdmin] = 0';

        if ($scope.UserGroupName != undefined && $scope.UserGroupName != "") {
            whereClause = "[RoleName] LIKE '%" + $scope.UserGroupName + "%'"
        }

        $http({
            url: encodeURI('/Role/GetRolePaged?StartRecordNo=' + StartRecordNo + '&RowPerPage=' + $scope.PerPage + '&whereClause=' + whereClause + '&rows=' + 0),
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
