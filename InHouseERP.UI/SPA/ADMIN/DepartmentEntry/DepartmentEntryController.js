app.controller("DepartmentEntryController", function ($scope, $rootScope, $http, $filter, $window) {
    //For Screen lock
    ...
    function Clear() {
        ...
        $scope.ScreenId = Permission.find(v => v.ScreenName == 'Department').ScreenId;
        ...
        $scope.ad_Department.OpeningDate = $filter('date')(new Date().toJSON().slice(0, 10), 'MMM dd, yyyy');
        $scope.button = "Save";
        $scope.Show = false;
    ...
    function GetDepartmentType() {
        $http({
            ...
            angular.forEach($scope.DepartmentTypelist, function (aDeptType) {
                if (aDeptType.DepartmentTypeName == 'Others') {
    ...
    function GetDepartmentPaged(curPage) {
        ...
        var SearchCriteria = "";
         ...
         if ($scope.SearchBranchAndDepartment !== undefined && $scope.SearchBranchAndDepartment != null && $scope.SearchBranchAndDepartment != "") {
             SearchCriteria = "[DepartmentName] LIKE '%" + $scope.SearchBranchAndDepartment + "%' OR [BranchName] LIKE '%" + $scope.SearchBranchAndDepartment + "%'";
            //alert("Name Success!!!!!");
        }
        $http({
            url: '/Department/GetDepartmentPaged?StartRecordNo=' + StartRecordNo + '&RowPerPage=' + $scope.PerPage + '&WhereClause=' + SearchCriteria + '&rows=' + 0,
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
    ...
    $scope.reloadBtn = function () {
        $('#textBranchAndDepartment').val('');
        $scope.SearchBranchAndDepartment = null;
        GetDepartmentPaged(1);

    }
    $scope.BranchDepartmentSearch = function () {
        GetDepartmentPaged(1);
    }
    ...
    $scope.SelDepartment = function (aDepartment) {
        ...
        if (res == "/Date") {
            ...
            aDepartment.OpeningDate = $filter('date')(parsedDate, 'MMM dd, yyyy');
        }
