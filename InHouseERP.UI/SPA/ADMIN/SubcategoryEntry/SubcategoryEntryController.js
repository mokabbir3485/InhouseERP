app.controller("SubcategoryEntryController", function ($scope, $rootScope, $http, $window) {
    //For Screen lock
    ...
    function Clear() {
        ...
        $scope.ScreenId = Permission.find(v => v.ScreenName == 'Price Type').ScreenId;
    ...
    function GetSubcategoryPaged(curPage) {
        ...
        $http({
            url: encodeURI('/Subcategory/GetSubcategoryPaged?StartRecordNo=' + StartRecordNo + '&RowPerPage=' + $scope.PerPage + '&SearchCr=' + SearchCriteria + '&rows=' + 0),
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
