app.controller("TruckChallanReportController", function ($scope, $cookieStore, $http, $filter, $window) {
    ...
    //$scope.CommercialInvoiceId = parseInt(sessionStorage.getItem("CommercialInvoiceIdTC"));
    $scope.CommercialInvoiceId = $cookieStore.get("CommercialInvoiceId");

    Clear();
    $scope.TableHtmlDataList = [];
    $scope.TableHtmlData = {};
    GetHTMLTableForReport('Truck');
    function Clear() {
    ...
    $scope.saveHtml = function () {
        ...
        if ($scope.TableHtmlDataList.length == 0) {
            ...
            $('#Tools').show();
            $('.alertify-logs').show();
            alertify.log("You can't save truck challan. Cause, You don't save  1st time delivery challan!", 'error', '5000');
            return;
        }
        if ($scope.TableHtmlDataList[0].DocType == 'Delivery') {
            $scope.TableHtmlDataList[0].DocType = 'Truck'
            $scope.TableHtmlData.DocType = 'Truck';
            $scope.TableHtmlData.Id = 0;
        } else {
        ...
        $http.post('/ExpCommercialInvoice/PostHTMLTableForReport', parms).success(function (data) {
            if (data > 0) {
                ...
                $('.alertify-logs').show();
                alertify.log('Truck Challan Report Save Successfully!', 'success', '5000');
                $scope.TableHtmlData = {};
            } else {
                $('.alertify-logs').show();
    ...
    function GetHTMLTableForReport(DocType) {
        $http({
            ...
            } else if (DocType == 'Truck' && $scope.TableHtmlDataList.length == 0) {
