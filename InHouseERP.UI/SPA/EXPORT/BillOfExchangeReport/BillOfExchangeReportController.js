...
app.controller("BillOfExchangeReportController", function ($scope, $route, $cookieStore, $http, $filter, $window) {
    var UserData = sessionStorage.getItem("UserDataSession");
    if (UserData != null) {
        $scope.LoginUser = JSON.parse(sessionStorage.UserDataSession);
    }
    //$scope.CommercialInvoiceId = parseInt(sessionStorage.getItem("CommercialInvoiceIdBE1"));
    $scope.CommercialInvoiceId = $cookieStore.get("CommercialInvoiceId");
    Clear();
    GetHTMLTableForReport('Bill1');
    function Clear() {
        $scope.TableHtmlData = {};
    ...
    $scope.saveHtml = function () {
        ...
        $scope.TableHtmlData.DocType = 'Bill1';
        ...
        $http.post('/ExpCommercialInvoice/PostHTMLTableForReport', parms).success(function (data) {
            if (data > 0) {
                $('.alertify-logs').show();
                alertify.log('Bill Of Exchange1 Certificate Report Save Successfully!', 'success', '5000');
                //Clear();
                $scope.TableHtmlData = {};
            } else {
                $('.alertify-logs').show();
