app.controller("DeliveryChallanReportController", function ($scope, $cookieStore, $http, $filter, $window) {
    ...
    //$scope.CommercialInvoiceId = parseInt(sessionStorage.getItem("CommercialInvoiceIdDCR"));
    ...
    $scope.saveHtml = function () {
        ...
        //$scope.TableHtmlData.HtmlItem = String($("#ciTable")[0].outerHTML);
        console.log('Full Report: ', $scope.TableHtmlData.HtmlData);
        $scope.TableHtmlData.DocType = 'Delivery';
        ...
        $http.post('/ExpCommercialInvoice/PostHTMLTableForReport', parms).success(function (data) {
            if (data > 0) {
                $('.alertify-logs').show();
                alertify.log('Delivery Challan Report Save Successfully!', 'success', '5000');
                $scope.TableHtmlData = {};
            } else {
                $('.alertify-logs').show();
    ...
    function GetHTMLTableForReport() {
        $http({
            url: '/ExpCommercialInvoice/GetHTMLTableForReport?DocumentId=' + $scope.CommercialInvoiceId + '&DocType=Delivery',
            method: 'GET',
