app.controller("SalarySheetSreepurController", function ($scope, $cookieStore, $http, $filter, $window) {
    ...
    var HrSalarySheet = {};
    HrSalarySheet = $cookieStore.get("HrSalarySheetSreepur");
    $scope.HrSalarySheet = $cookieStore.get("HrSalarySheetSreepur");
                ...
                Object.keys(aSalarySheet).forEach(key => {
                    if (!isNaN(Number(aSalarySheet[key]))) {
                        if (aSalarySheet[key] !== '' && aSalarySheet[key] != null) {
                            if (key != 'EmployeeCode' & key != 'TotalDays') {
